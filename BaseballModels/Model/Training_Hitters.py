...
if __name__ == "__main__":
    ...
    for i in tqdm(range(num_models), desc="Training Hitter Models"):
        best_loss = 10
        while best_loss > 8: # Throw away trainings that get stuck in local minima
            ...
            best_loss = Model_Train.trainAndGraph(network, training_generator, testing_generator, len(train_hitters_dataset), len(test_hitters_dataset), loss_function, optimizer, scheduler, num_epochs, logging_interval=10000, early_stopping_cutoff=40, should_output=False, model_name=f"Models/{model_name}.pt")

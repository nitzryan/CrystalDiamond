...
if __name__ == "__main__":
    ...
    for model_name, model_id in tqdm(model_idxs, desc="Evaluating Architectures"):
        ...
        network_mlb = RNN_Model(x_padded[0].shape[1], num_layers, hidden_size, None, data_prep=data_prep, is_hitter=False)
        network_prospect = RNN_Model(x_padded[0].shape[1], num_layers, hidden_size, None, data_prep=data_prep, is_hitter=False)
                ...
                for player_idx in range(len(pt_values)):
                    ...
                    for date_idx in range(pt.size(0)):
                        ...
                        for level_idx in range(NUM_LEVELS):
                            outs_sp, outs_rp, gs, g = tuple(pt[date_idx, level_idx].tolist())
                            if (outs_sp + outs_rp) > 20:
                                pa_list.append((outs_sp, outs_rp, gs, g))
                    ...
                    cursor.executemany(f"INSERT INTO Output_PitcherStats VALUES({mlbId}, {model_id}, {model_idx}, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", vals)

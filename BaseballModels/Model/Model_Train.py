...
PWAR_LOSS_MULTIPLIER = 1.0
LEVEL_LOSS_MULTIPLIER = 0.8
PA_LOSS_MULTIPLIER = 1.0
STATS_LOSS_MULTIPLIER = 0.6
POSITION_LOSS_MULTIPLIER = 1.0
TWAR_LOSS_MULTIPLIER = 0.6
VALUE_LOSS_MULTIPLIER = 0.6
LOSS_ITEM = 0
...
def train(network,  data_generator, num_elements, loss_function, loss_function_stats, loss_function_position, optimizer, logging = 200, should_output=True):
  ...
  num_batches = 0
  for batch, (data, length, target_war, target_value, target_pwar, target_level, target_pa, target_stats, target_positions, mask_labels, mask_stats, year_mask, year_stats, year_position) in enumerate(data_generator):
    ...
    optimizer.zero_grad()
    output_war, output_value, output_pwar, output_level, output_pa, output_stats, output_position, output_yearStats, output_yearPos = network(data, length)
    ...
    loss_position.backward(retain_graph=True)
    ...
    avg_loss[4] += loss_stats.item() / STATS_LOSS_MULTIPLIER
    avg_loss[5] += loss_position.item() / POSITION_LOSS_MULTIPLIER
    avg_loss[6] += loss_yearStats.item() / STATS_LOSS_MULTIPLIER
    avg_loss[7] += loss_yearPos.item() / POSITION_LOSS_MULTIPLIER
    avg_loss[8] += loss_value.item() / VALUE_LOSS_MULTIPLIER
    num_batches += 1
...
def test(network, test_loader, num_elements, loss_function, loss_function_stats, loss_function_position):
  ...
  num_batches = 0
  with torch.no_grad():
    for data, length, target_war, target_value, target_pwar, target_level, target_pa, target_stats, target_positions, mask_labels, mask_stats, year_mask, year_stats, year_position in test_loader:
      data, length = data.to(device), length.to(device)
      target_war, target_value, target_pwar, target_level, target_pa = target_war.to(device), target_value.to(device), target_pwar.to(device), target_level.to(device), target_pa.to(device)
      target_stats = target_stats.to(device)
      target_positions = target_positions.to(device)
...
def trainAndGraph(network, training_generator, testing_generator, num_train : int, num_test : int, loss_function, loss_function_stats, loss_function_position, optimizer, scheduler, num_epochs, logging_interval=1, early_stopping_cutoff=20, should_output=True, graph_y_range=None, model_name="no_name.pt"):
  ...
  if should_output:
    ...
    graphLoss(epoch_counter, train_loss_history[4], test_loss_history[4], start=5, title="Stats Prediction")
    graphLoss(epoch_counter, train_loss_history[5], test_loss_history[5], start=5, title="Position Prediction")
    graphLoss(epoch_counter, train_loss_history[6], test_loss_history[6], start=5, title="Year Stats Prediction")
    graphLoss(epoch_counter, train_loss_history[7], test_loss_history[7], start=5, title="Year Position Prediction")
    graphLoss(epoch_counter, train_loss_history[8], test_loss_history[8], start=5, title="Value Prediction")
  return best_loss
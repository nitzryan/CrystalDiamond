...
def train(network,  data_generator, num_elements, loss_function, loss_function_stats, loss_function_position, optimizer, logging = 200, should_output=True):
  ...
  num_batches = 0
  for batch, (data, length, target_war, target_pwar, target_level, target_pa, target_stats, target_positions, mask_labels, mask_stats, year_mask, year_stats, year_position) in enumerate(data_generator):
    ...
    optimizer.zero_grad()
    output_war, output_pwar, output_level, output_pa, output_stats, output_position, output_yearStats, output_yearPos = network(data, length)
    ...
    avg_loss[5] += loss_position.item()
    avg_loss[6] += loss_yearStats.item()
    avg_loss[7] += loss_yearPos.item()
    num_batches += 1
...
def test(network, test_loader, num_elements, loss_function, loss_function_stats, loss_function_position):
  ...
  num_batches = 0
  with torch.no_grad():
    for data, length, target_war, target_pwar, target_level, target_pa, target_stats, target_positions, mask_labels, mask_stats, year_mask, year_stats, year_position in test_loader:
      data, length = data.to(device), length.to(device)
      target_war, target_pwar, target_level, target_pa = target_war.to(device), target_pwar.to(device), target_level.to(device), target_pa.to(device)
      target_stats = target_stats.to(device)
      target_positions = target_positions.to(device)

...
base_prep_map = Prep_Map(
    ...
    map_hitterlvl=lambda h : [h.Age, h.LevelId, h.Month, h.LeagueAverageAge],
    map_pitcherlvl=lambda p : [p.Age, p.LevelId, p.Month, p.LeagueAverageAge],
...
meanrregression_prep_map = Prep_Map(
    map_bio=lambda p : [p.ageAtSigningYear, math.log10(p.draftPick), math.log10(p.draftSignRank)],
    map_off=lambda h : (f := min(h.PA / 50, 1), [_MeanRevert(f, h.ParkRunFactor), 
    ...
    map_hitterlvl=lambda h : [h.Age, h.LevelId, h.Month],
    map_pitcherlvl=lambda p : [p.Age, p.LevelId, p.Month],
    map_pit=lambda p : (f := min(p.BF/ 50, 1), 

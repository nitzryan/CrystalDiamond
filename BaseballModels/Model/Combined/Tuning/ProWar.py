...
def objective(
    ...
    input_noise = trial.suggest_float('input_noise', 0, 0.5)
    ...
    lr_shared = trial.suggest_float("lr_shared", 1e-4, 1e-1, log=True)
    ...
    wd_shared = trial.suggest_float("wd_shared", 1e-5, 1e-1, log=True)
    ...
    for i in range(max_repeats):
        ...
        if (current_mean > _WAR_CUTOFF and is_hitter) or (current_mean > _WAR_CUTOFF_P and not is_hitter):
            break
        
    return min(np.mean(metrics), _MAX_LOSS if is_hitter else _MAX_LOSS_P)
...
def Train_Pitchers(num_models : int):
    ...
    for model_name, model_id in tqdm(model_idxs, desc="Training Pitching Architectures"):
        ...
        pitcher_io_list = data_prep.Generate_IO_Pitchers(is_training=True)
        ...
        for i in tqdm(range(num_models), desc="Training Pitcher Models", leave=False):
            ...
            train_dataset, test_dataset = Create_Test_Train_Datasets(player_list=pitcher_io_list, is_hitter=False, train_idx=i)
            ...
            train_results = TrainAndGraph(
                ...
                pro_model_name=f"Model/Models/pro_{model_name_pt}_pit",
                col_model_name=f"Model/Models/col_{model_name_pt}_pit",
                is_hitter=False,
                should_output=False,
            ...
            model_cursor.execute("INSERT INTO Model_TrainingHistory VALUES (?,?,?,?,?,?,?,?,?)", (model_name, 0, train_results.best_loss, train_results.test_losses[loss_index][-1], i, pro_network.GetNumLayers(), pro_network.GetHiddenSize(), col_network.GetNumLayers(), col_network.GetHiddenSize()))

...
def Eval_Hitters(eval_update : bool):
    with torch.no_grad():
        ...
        if eval_update:
            ...
            year, month = cursor.execute("SELECT year, month FROM Output_PlayerWarAggregation WHERE isHitter=1 ORDER BY year DESC, month DESC LIMIT 1").fetchone()
            month += 1
            if month > 9:
                month = 4
                year += 1
            
            college_year, = cursor.execute("SELECT year FROM Output_College_Hitter ORDER BY year DESC LIMIT 1;").fetchone()
            college_year += 1
        else:
        ...
        for i in range(1, len(TOTAL_WAR_BUCKETS)):
            ...
            war_bucket_averages.append(base_db.execute(f"SELECT AVG(warHitter) FROM Model_Players WHERE IsEligible=1 AND IsHitter=1 AND warHitter>{bucket_min} AND warHitter<={bucket_max}").fetchone()[0])
        ...
        for model_name, model_id in tqdm(model_idxs, desc="Evaluating Hitting Architectures"):
            ...
            if eval_update:
                hitter_io_list = data_prep.Generate_IO_Hitters_Update(year, month, college_year)
            else:
                hitter_io_list = data_prep.Generate_IO_Hitters(is_training=False)

...
if __name__ == "__main__":
    ...
    for model_name, model_id in tqdm(model_idxs, desc="Training Architectures"):
        ...
        pro_mlb_value_mean : torch.Tensor = data_prep.pro_data_prep.__getattribute__('__pitchervalues_means').to(device)
        pro_mlb_value_stds : torch.Tensor = data_prep.pro_data_prep.__getattribute__('__pitchervalues_devs').to(device)
        ...
        pro_war_means : torch.Tensor = getattr(data_prep.pro_data_prep, "__pit_prospect_value_means").to(device)
        pro_war_devs : torch.Tensor = getattr(data_prep.pro_data_prep, "__pit_prospect_value_devs").to(device)

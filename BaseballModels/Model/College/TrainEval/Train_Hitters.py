...
if __name__ == "__main__":
    ...
    model_idxs = model_cursor.execute("SELECT hitterModelName, id FROM ModelIdx_College ORDER BY id ASC").fetchall()
    
    for model_name, model_id in tqdm(model_idxs, desc="Training Architectures"):
        model_cursor = model_db.cursor()
        model_cursor.execute("DELETE FROM PlayersInTrainingData_College WHERE modelIdx=?", (model_id,))
        ...
        hitter_io_list = data_prep.Generate_IO_Hitters("WHERE LastYear<=? AND isHitter=?", (2017, 1), use_cutoff=True)
        ...
        model_cursor = model_db.cursor()
        model_cursor.executemany("INSERT INTO PlayersInTrainingData_College VALUES(?,?)", [(io.player.TBCId,model_id) for io in hitter_io_list])
        model_db.commit()
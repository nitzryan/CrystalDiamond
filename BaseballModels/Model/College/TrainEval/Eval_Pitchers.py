...
if __name__ == "__main__":
    ...
    cursor.execute("DELETE FROM Output_College WHERE isHitter=0")
    model_db.commit()
    cursor = model_db.cursor()
    model_idxs = cursor.execute("SELECT pitcherModelName, id FROM ModelIdx_College ORDER BY id ASC").fetchall()
    ...
    for model_name, model_id in tqdm(model_idxs, desc="Evaluating Architectures"):
        ...
        pitcher_io_list = data_prep.Generate_IO_Pitchers("WHERE isPitcher=?", (1,), use_cutoff=False)
        eval_dataset = College_Eval_Dataset(pitcher_io_list)
        ...
        network = RNN_Model(eval_dataset.GetInputSize(), 
                            ...
                            hidden_size=hidden_size, data_prep=data_prep, is_hitter=False)
        ...
        for m in tqdm(mth, desc="Evaluating Model Copies", leave=False):
            ...
            for (data, lengths, dates) in tqdm(generator, total=len(generator), desc="Evaluating Pitchers", leave=False):
                ...
                for d in db_input:
                    ...
                    cursor.executemany(f"INSERT INTO Output_College VALUES(?,{model_id},0,?,?,?,?,?,?,?,?,?,?)", vals)

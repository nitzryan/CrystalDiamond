    ...
    for model_name, model_id in tqdm(model_idxs, desc="Evaluating Architectures"):
        ...
        for m in tqdm(mth, desc="Evaluating Model Copies", leave=False):
            ...
            for (data, lengths, dates) in tqdm(generator, total=len(generator), desc="Evaluating Hitters", leave=False):
                ...
                output_draft, output_war, output_off, output_def, output_pa, output_pos = network(data, lengths)
                
                # Predictions
                output_draft = F.softmax(output_draft, dim=-1)
                output_war = F.softmax(output_war, dim=-1)
                output_off = F.softmax(output_off, dim=-1)
                output_def = F.softmax(output_def, dim=-1)
                output_pa = F.softmax(output_pa, dim=-1)
                ...
                db_input = torch.cat((ids, model_idxs, years, output_draft, draftMean.unsqueeze(-1), output_war, warMean.unsqueeze(-1), output_off, output_def, output_pa, output_pos), dim=2)

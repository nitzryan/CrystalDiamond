...
if __name__ == "__main__":
    ...
    cursor.execute("DELETE FROM Output_WarQuants WHERE isHitter=1")
    db.commit()
    cursor = db.cursor()
    ...
    for model_name, model_id in tqdm(model_idxs, desc="Evaluating Architectures"):
        ...
        pt_levelYearGames = torch.nn.utils.rnn.pad_sequence([io.pt_levelYearGames for io in hitter_io_list])
        mask_prospect_padded = torch.nn.utils.rnn.pad_sequence([io.prospect_mask for io in hitter_io_list])
        eval_hitters_dataset = Eval_Dataset(x_padded, lengths, pt_levelYearGames, dates_padded, mask_prospect_padded)
        batch_size = 1000
        ...
        war_means : torch.Tensor = getattr(data_prep, "__hit_prospect_value_means").to(device)
        war_devs : torch.Tensor = getattr(data_prep, "__hit_prospect_value_devs").to(device)

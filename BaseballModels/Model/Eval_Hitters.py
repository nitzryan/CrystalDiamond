...
from Hitter_Model import LSTM_Model
...
if __name__ == "__main__":
    ...
    # Setup Model
    cursor = db.cursor()
    mth = DB_Model_TrainingHistory.Select_From_DB(cursor, "WHERE ModelName=?", ("Hitter",))
    ...
    for m in tqdm(mth, desc="Evaluation Models"):
        ...
        for batch, (data, length, dtes) in tqdm(enumerate(generator), total=len(generator), desc="Evaluating Hitters", leave=False):
            ...
            for dbd in db_data:
                ...
                cursor.executemany("INSERT INTO Output_PlayerWar VALUES(?,?,?,?,?,?,?,?,?,?,?)", vals)
            db.commit()

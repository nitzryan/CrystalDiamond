...
class DB_PitchData:
	def __init__(self, values : tuple[any]):
		...
		self.PitcherId = values[7]
		self.PitchType = values[8]
		self.PitchClass = values[9]
		self.CountBalls = values[10]
		self.CountStrike = values[11]
		self.PitIsR = values[12]
		self.HitIsR = values[13]
		self.Result = values[14]
		self.HadSwing = values[15]
		self.HadContact = values[16]
		self.IsInPlay = values[17]
		self.RunValueInPlay = values[18]
		self.Vel = values[19]
		self.Extension = values[20]
		self.BreakInduced = values[21]
		self.BreakHorizontal = values[22]
		self.SpinRate = values[23]
		self.SpinAxis = values[24]
		self.ActiveSpin = values[25]
		self.VaaAboveAverage = values[26]
		self.HaaAboveAverage = values[27]
		self.PlateX = values[28]
		self.PlateZ = values[29]
		self.ZoneTop = values[30]
		self.ZoneBot = values[31]

	NUM_ELEMENTS = 32

                            
	def To_Tuple(self) -> tuple[any]:
		return (self.GameId,self.PitchId,self.Year,self.LevelId,self.Month,self.RunValueSmoothedHitter,self.RunValueHitter,self.PitcherId,self.PitchType,self.PitchClass,self.CountBalls,self.CountStrike,self.PitIsR,self.HitIsR,self.Result,self.HadSwing,self.HadContact,self.IsInPlay,self.RunValueInPlay,self.Vel,self.Extension,self.BreakInduced,self.BreakHorizontal,self.SpinRate,self.SpinAxis,self.ActiveSpin,self.VaaAboveAverage,self.HaaAboveAverage,self.PlateX,self.PlateZ,self.ZoneTop,self.ZoneBot)

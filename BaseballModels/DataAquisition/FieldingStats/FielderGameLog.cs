...
namespace DataAquisition.FieldingStats
{
    internal class FielderGameLog
    {
        ...
        private static async Task<List<Player_Fielder_GameLog>> GetFielderGameLogsAsync(int id, HttpClient httpClient, int year)
        {
            ...
            foreach (var split in splits)
            {
                ...
                int[] inningsValues = [.. inningsElement.GetString().Split('.').Select(f => int.Parse(f))];
                ...
                #pragma warning disable CS8602 // Will exist or expect exception
                int[] date = [.. dateElement.GetString().Split('-').Select(f => int.Parse(f))];
                ...
                logs.Add(new Player_Fielder_GameLog
                {
                    ...
                    Month = date[1],
                    Year = year,
                    Position = (DbEnums.Position)int.Parse(codeElement.GetString()),
